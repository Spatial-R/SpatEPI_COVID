...
fixed_p <- unlist(lapply(measles_unit_statenames[1:(length(measles_unit_statenames)-1)], function(data){
  ...
  if(!length(fix_col) == 0){
    if(data == "S"){
      ...
      test_p <- rep(1e-7,length(fix_col))
    }
...
write.csv(dat_day,file = "Process_Data/City_Network_Intervention_Weak_Total.csv",row.names = F)

save(origin_model,origin_model_ct,origin_model_fc,origin_model_fcct,
     file ="E:/nCOV/Process_Data/City_Net_Intervention_Weak_Total.RData")
