...
for (pro_n in c("guangzhou","shenzhen","wenzhou","xinyang","changsha","nanchang")){

### 


load(paste("Cities_Spatial_",pro_n,".RData",sep = ""))
...
simlu <- read.csv("Hope_UPQ_NF_IVP_K1.csv",header=T) %>% arrange(desc(loglik));
...
measles_params$S18_0 <- measles_params$S15_0
measles_params$E18_0 <- measles_params$E15_0
measles_params$I18_0 <- measles_params$I15_0
measles_params$R18_0 <- measles_params$R15_0
measles_params$P18_0 <- measles_params$P15_0
measles_params$J18_0 <- measles_params$J15_0
measles_params$quatar <- c(0.2,0.6)

measles_params <- measles_params[unlist(lapply(measles_paramnames,function(id)which(names(measles_params) ==id)))]
...
fix_col <- c(1:13)[!measles_paramnames[c(1:13)] %in% c("alpha","g","sigma","gamma","qualow","R0IN","kappa","omega","qua","quawh")]
rw_text <- paste(c(paste(measles_paramnames[fix_col],"=",rw.sd_rp,sep = ""),
                   paste(measles_paramnames[-c(1:13)],"=",rw.sd_ivp,"",sep = "")),collapse = ",")
...
write.csv(global.result,file=paste("Hope_UHO_PRO_IVP_L_",pro_n,".csv",sep=""),row.names = F)
...
names(case_simu_low)[-1] <- names(case_simu_median)[-1] <- names(case_simu_high)[-1]  <- unique(case_all_final$city)
...
case_plot <- merge(case_all_final,case_plot,by = c("day","city"))
...
ggsave(paste("Hope_UHO_PRO_IVP_L_",pro_n,".pdf",sep=""),width = 22,height = 16,units = "cm",dpi = 300)
}