...
spatPomp(ncov_cases,
         ...
         partrans=parameter_trans(log=c("alpha","g","R0IN","kappa"),
                                  logit = c("psi","qua","sigma","gamma","quawh","qualow","rho","omega",
                                            paste0("S",1:U,"_0"),
                                            paste0("E",1:U,"_0"),
                                            paste0("J",1:U,"_0"),
                                            paste0("P",1:U,"_0"),
...
#for (i in seq(1.5,3.5,0.5)){

global.result <- read.csv("Process_Data/Hope_UPQ_NF_IVP_K1.csv",header=T) %>% arrange(desc(loglik));
...
source("Codes/help_functions.R")

case_plot <- mutate(case_plot,day  = as.Date(day),city = firstup(city))
...
ggplot(case_plot,aes(x = day,y = median,ymin = low,ymax = high))+
  ...
  theme_classic(base_size = 10,base_family = "serif") + xlab("") +
  ylab("Daily new confirmed COVID-19 cases")

ggsave("Figures/Hubei_Model_Simulation.pdf",
